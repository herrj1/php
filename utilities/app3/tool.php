...
//echo "DBDEF variable: ". $DBDEF['db'] ."<br>";
...
'host' => '127.0.0.1',
'port' => '3306',
); #working copy for DB settings
//echo "DB variable : ". $DB['db'] ."<br>";
...
$page=$_REQUEST['p']+0;
if ($_REQUEST['refresh'] && $DBDEF['db'] && preg_match('/^show/',$SQLq) ) $SQLq=$SHOW_T;
...
if (db_connect('nodie')){
  ...
  if ($_REQUEST['pi']){
   ...
   //echo 'Get initial values: '. $DBDEF['db'].'<br>';
   //if($DB['db']){
   if ($DBDEF['db']){
    if ($_REQUEST['shex']){
     print_export();
...
function display_select($sth,$q){
 ...
 //global $dbh,$DBDEF,$sqldr,$reccount,$is_sht,$xurl,$is_sm;
 ...
 <div class='col-md-12'><table id='res' class='table table-bordered table-responsive'>";
 $headers="<tr class='h'>";
 ...
 if ($is_sht) $headers.="<th>Engine</th><th>~Rows</th><th>Data size</th><th>Index size</th><!--<th>show create table</th><th>explain</th><th>indexes</th>--><th>Action</th><!--<th>drop</th><th>truncate</th><th>optimize</th><th>repair</th><th>comment</th>-->";
 $headers.="</tr>\n";
 $sqldr.=$headers;
 $swapper=false;
 while($row=mysqli_fetch_row($sth)){
   ...
   if ($is_sht){
     ...
     ."<td><a class='btn btn-info' href=\"$url&shex=1&rt=".hs(ue($vq))."\">Export</a></td>"
...
function print_header(){
 ...
 global $err_msg,$VERSION,$DBDEF,$dbh,$self,$is_sht,$xurl,$SHOW_T;
 //$dbn=$DB['db'];
 $dbn=$DBDEF['db'];
 //echo "Print header: ". $dbn;
?>
<!DOCTYPE html>
<html>
<head><title>Jonathan's SQL Web Querying Tool</title>
<meta charset="utf-8">
...
<?php echo $z.'&q='.b64u($SHOW_T)?>' class="btn btn-outline-primary" >show tables</a><!--
<?php echo $z?>&shex=1' class="btn btn-outline-secondary">Export Database</a>
<?php echo $z?>&shim=1' class="btn btn-outline-info">import</a>-->
<?php } ?>
...
function print_screen(){
 ...
 if ($is_limited_sql && ($page || $reccount>=$MAX_ROWS_PER_PAGE) ){
  $nav="<div class='col-md-12'>".get_nav($page, 10000, $MAX_ROWS_PER_PAGE, "javascript:go(%p%)")."</div>";
 }
...
<div class="col-md-12">
<label for="qraw"><b>Jonathan's SQL-Web Querying Tool</b></label>
<!--<div class="btn-group btn-group-sm" role="group" aria-label="left_or_right">
  <button type="button" class="btn btn-secondary" onclick="q_prev()">&lt;</button>
  <button type="button" class="btn btn-secondary" onclick="q_next()">&gt;</button>
</div>-->

<textarea id="qraw" cols="70" rows="10"><?php eo($SQLq)?></textarea><br>
<input type="hidden" name="q" id="q" value="<?php b64e($SQLq);?>">
<input type="submit" name="GoSQL" value="Go" class="btn btn-primary">
<input type="button" name="Clear" value=" Clear " class="btn btn-warning" onclick="$('qraw').value='';" style="width:100px">
<?php if(!empty($_REQUEST['db'])){ ?>
<div style="float:right">
<input class="btn btn-outline-success" type="button" value="Select" class="sbtn" onclick="qtpl('SELECT *\nFROM %T\nWHERE 1')">
<!--<input type="button" value="Insert" class="sbtn" onclick="qtpl('INSERT INTO %T (`column`, `column`)\nVALUES (\'value\', \'value\')')">-->
<input class="btn btn-outline-warning" type="button" value="Update" class="sbtn" onclick="qtpl('UPDATE %T\nSET `column`=\'value\'\nWHERE 1=0')">
...
<div class="col-md-12">&copy; <?php echo Date('Y') ?> <a href="#" target="_blank">Jonathan's Web SQL Querying Tool</a></div>
</div></div>
...
function print_cfg(){
 ...
 //global $DBDEF,$err_msg,$self;
 //echo "Print_cfg ". $DB['db'];
 print_header();
?>
<center>
...
<label><div class="l">Charset:</div><select name="v[chset]"><option value="">- default -</option><?php echo chset_select($DBDEF['chset'])?></select></label><br>
...
function db_connect($nodie=0){
 ...
 global $dbh,$DBDEF,$err_msg;

 if ($DBDEF['port']) {
    $dbh=mysqli_connect($DBDEF['host'],$DBDEF['user'],$DBDEF['pwd'],'',(int)$DBDEF['port']);
 } else {
    $dbh=mysqli_connect($DBDEF['host'],$DBDEF['user'],$DBDEF['pwd']);
 }
 if (!$dbh) {
 ...
 if ($dbh && $DBDEF['db']) {
  $res=mysqli_select_db($dbh, $DBDEF['db']);
  if (!$res) {
     $err_msg='Cannot select db because: '.mysqli_error($dbh);
     if (!$nodie) die($err_msg);
  }else{
     if ($DBDEF['chset']) db_query("SET NAMES ".$DBDEF['chset']);
  }
 }
...
function print_export(){
 ...
 //global $self,$xurl,$DBDEF,$DUMP_FILE;
// echo "Hello from print export:". $DB['db'];
 $t=$_REQUEST['rt'];
 //echo "Printing table ". $t;
 $l=($t)?"Table $t":"whole DB";
 echo '<div class="row"><div class="col-md-6 offset-md-3 text-center">';
 print_header();
?>

<h1>Export <?php eo($l)?></h1>
<div class="col-md-6 offset-md-3">
  <input class="form-check-input" type="checkbox" name="s" value="1" checked >
  <label class="form-check-label" >Structure</label>

</div>
<div class="col-md-6 offset-md-3">
  <input class="form-check-input" type="checkbox" name="d" value="1" checked>
  <label class="form-check-label" >Data</label>
</div>
	<!--<input class="form-check-input" type="checkbox" name="s" value="1" checked> Structure<br>
	<input class="form-check-input" type="checkbox" name="d" value="1" checked> Data<br><br>-->

<!--<div>
<label><input class="form-check-input" type="radio" name="et" value="" checked> .sql</label>&nbsp;</div>-->

<div class="col-md-12 text-center">
  <input class="form-check-input" type="radio" name="et" value="" checked>
  <label class="form-check-label" for="inlineRadio1">SQL</label>
</div>
...
<?php if ($t && !strpos($t,',')){?>
 ...
 <div class="col-md-12 text-center">
  <input class="form-check-input" type="radio" name="et" value="csv"> 
...
<label class="form-check-label"> import has super privileges</label>
</div>

<div class="col-md-12">
<input type="checkbox" name="gz" value="1">
<label class="form-check-label">compress as .gz</label>
</div>
<br>
...
<input class="btn btn-warning" type="submit" name="srv" value=" Dump on Server ">
<input class="btn btn-danger" type="button" value=" Cancel " onclick="window.location='<?php eo($self.'?'.$xurl.'&db='.ue($DB['db']))?>'">
...
function do_export(){
 ...
 //global $DBDEF,$VERSION,$D,$BOM,$ex_isgz,$ex_issrv,$dbh,$out_message;
 ...
 if ($ct==1&&$_REQUEST['et']=='csv'){
  ...
  ex_w("-- fauconnect2 dump $VERSION$D-- Datetime: ".date('Y-m-d H:i:s')."$D-- Host: $DB[host]$D-- Database: $DB[db]$D$D");
  ...
  //$sth=db_query("show full tables from `$DBDEF[db]`");
  while($row=mysqli_fetch_row($sth)){
